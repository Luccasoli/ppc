        ...
        with condition:
            ...
            if randint(0, 1):
                if carros_gerados < N_CARROS/2:
                    ...
                    carro = Carro('{}_{}'.format(carros_gerados+1, origem.upper()), TRAVESSIA_CARRO, origem, 'carro', 2)
            ...
            # GERA CAMINHÃO
                ...
                if caminhoes_gerados < N_CAMINHOES/2:
                    sleep(randint(TA[0], TA[1]))
                    caminhao = Caminhao('{}_{}'.format(caminhoes_gerados+1, origem.upper()), TRAVESSIA_CAMINHAO, origem, 'caminhao')
...
def atravessa(**kwargs):
    ...
    with ponte_sync:
        ...
        while(ponte.total_atravessou_para(destino) < (N_CAMINHOES+N_CARROS)/2):
            if ponte.ocupada:
                print("*** PONTE OCUPADA ***")
                ponte_sync.wait()
            ...
            with condition:
                ...
                print("UM {} VAI ATRAVESSAR!".format(veiculo.tipo))
                print('{} - {} - ATRAVESSANDO para {}'.format(veiculo.tipo, veiculo.id, destino))
                veiculo.atravessar() # SLEEP
                ponte.atravessou(veiculo.origem, veiculo.tipo) # INCREMENTA NA PONTE
...
def main():
    ...
    travessia_para_direita = Thread(target=atravessa, kwargs=
                                            {'fila': qe,
